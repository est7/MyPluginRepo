...
> 来源：`1st-cc-plugin/workflows/loaf/docs/gemini-roadmap-review/deepdive-gastown.md`
> 补充内容：Nudge 三种投递模式、原子 drain 机制、gt prime hook 细节、Mail/Wisp 整合实现。

### A.1 `gt nudge` 的三种投递模式

`gt nudge` 不是简单的消息发送，而是根据 agent 状态选择不同投递策略：

1. **`immediate`**：通过 `tmux send-keys` 直接向目标 session 注入文本，**可中断正在执行的 tool call**
2. **`queue`**：在 `.runtime/nudge_queue/<session>/` 目录下创建 JSON 文件，文件名为纳秒精度时间戳（确保 FIFO 排序）
3. **`wait-idle`**：先轮询 tmux pane 判断 agent 是否空闲，空闲则切换为 `immediate`；超时后降级为 `queue`

### A.2 Nudge Queue 的原子 Drain 机制

Agent 侧消费 nudge queue 时，使用**原子 rename** 防止竞争条件：
- 将 `.runtime/nudge_queue/<session>/<timestamp>.json` rename 为 `.claimed.<random>`
- rename 是原子操作——另一个进程无法同时 claim 同一消息
- 处理完成后删除 `.claimed.*` 文件

### A.3 `gt prime` Hook 模式

`gt prime --hook` 实现 session 启动时的工作注入：

1. **输入**：从 stdin 读取 JSON（`session_id`, `source` 字段）
2. **Identity 锁定**：`lock.New()` 确保同一时刻只有一个 prime 进程处理同一 session
3. **Work 发现**：`findAgentWork()` 查询 Beads DB，获取分配给当前 agent 的待办事项
4. **环境注入**：将工作上下文导出为 tmux 环境变量（`GT_WORK_RIG`、`GT_WORK_BEAD`、`GT_WORK_MOL`）
5. **Output Directives**：向 agent 的 prompt 中注入 `## 🚨 AUTONOMOUS WORK MODE 🚨` 标记块
6. **快速路径**：当 `source` 为 `compact` 或 `resume` 时，跳过 DB 查询直接返回（避免频繁重启场景下的性能损耗）

### A.4 Mail 与 Wisp 的数据层

- **`gt mail`**：通过 Beads DB 的 `gt:message` label 查询 issues，支持 `assignee` 和 `cc` 过滤
- **Wisp（ephemeral tracking）**：直接执行 Dolt SQL 查询（非 Git-based），用于短期任务追踪
- **Legacy `inbox.jsonl`**：仍保留的文件式收件箱，使用 `flock` 保护并发写入
