...
> 来源：`1st-cc-plugin/workflows/loaf/docs/gemini-roadmap-review/deepdive-spec-kit.md`
> 补充内容：process_template 机制、workflow engine、upsert_context markers 的具体实现。

### A.1 `process_template` 模板处理管线

spec-kit 的模板不是简单的字符串替换，而是多阶段处理流水线：

1. **Script 提取**：模板中的 `{{script:...}}` 块被提取并执行，输出替换回模板
2. **Placeholder 替换**：`{{var_name}}` 占位符用 context dict 的值填充
3. **路径重写**：模板中的相对路径自动转换为当前项目的绝对路径
4. **Frontmatter 剥离**：YAML frontmatter 在注入 agent prompt 前被移除（保留 metadata 供工具链使用）

### A.2 Integration Registry 的实际规模

`INTEGRATION_REGISTRY` 维护 agent key → `IntegrationBase` 子类的映射：
- 20+ 集成包括 Claude、Cursor、Windsurf、Codex 等主流 AI 编码助手
- 每个 `IntegrationBase` 子类实现 `install()`、`uninstall()`、`detect()` 接口
- `ClaudeIntegration` 特化：读取 `.claude/settings.json`，使用 `allowedTools` 白名单

### A.3 Workflow Engine（`engine.py`）

spec-kit 的 workflow 执行引擎支持：
- **Sequential 步骤**：默认模式，按声明顺序执行
- **`fan_out`**：并行执行多个独立步骤
- **`gate`**：条件检查点，失败则阻塞后续步骤
- **`retry`**：失败步骤的自动重试（含指数退避）
- **状态追踪**：workflow 执行状态持久化到 `state.json`

### A.4 `upsert_context_section` 与 SPECKIT Markers

Context 注入使用幂等的标记系统：
- **Start marker**：`<!-- SPECKIT:CONTEXT:START:{section_name} -->`
- **End marker**：`<!-- SPECKIT:CONTEXT:END:{section_name} -->`
- **Upsert 语义**：若 marker 对存在 → 替换中间内容；不存在 → 追加到文件末尾
- 确保多次执行 `spec-kit context update` 不会产生重复内容
